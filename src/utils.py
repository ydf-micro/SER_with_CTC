...
class Constants:
    def __init__(self):
        ...
        self.path_to_data = "../data/sessions/"
        self.path_to_features = "../data/features/"
    ...
    def __str__(self):
        def display(objects, positions):
            line = ''
            line += str(objects[0]) + ' ' * (positions - len(objects[0])) + str(objects[1])
            return line
...
def get_audio(path_to_wav, filename, params):
    wav = wave.open(path_to_wav + filename, mode="r")
    # 声道数、位数、采样频率、采样点数、压缩类型、压缩类型描述
...
def get_emotions(path_to_emotions, filename):
    emo_details = []
    with open(path_to_emotions + filename, 'r') as f:
        line = f.readline()
        while line:
            if line[0] == '[':
                line = re.sub(r'[\[\],]', '', line)
                details = line.split()
                emo_details.append({'start': float(details[0]),
                                    'end': float(details[2]),
                                    'id': details[3],
                                    'emotion': details[4],
                                    'v': float(details[5]),
...
def split_wav(wav, emotions, transcriptions, params):
...
def read_iemocap_data(params):
    ...
    for session in params.sessions:
        ...
        for f in files:
            wav = get_audio(path_to_wav, f + '.wav', params)
            transcriptions = get_transcriptions(path_to_transcriptions, f + '.txt')
            emotions = get_emotions(path_to_emotions, f + '.txt')
            data.extend(split_wav(wav, emotions, transcriptions, params))
